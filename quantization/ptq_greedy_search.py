...
from . import LinearQuantMode
from .range_linear import PostTrainLinearQuantizer, ClipMode
from utils.summary_graph import SummaryGraph
from utils.model_transforms import fold_batch_norms
import modules
from data_loggers import collect_quant_stats
...
def ptq_greedy_search(model, dummy_input, eval_fn, calib_eval_fn=None,
    ...
    if args.resume_search_from:
        with open(args.resume_search_from, 'r') as f:
            best_overrides_dict = utils.yaml_ordered_load(f)
    ...
    if not act_stats:
        ...
        model_temp = utils.utils.make_non_parallel_copy(model)
        ...
        if args:
            ...
            utils.yaml_ordered_save(act_stats_path, act_stats)
    ...
    def recalibrate_stats(module_name, act_stats):
        ...
        if not modules_to_recalibrate:
            ...
            return act_stats
        q = PostTrainLinearQuantizer(utils.make_non_parallel_copy(model),
                                     bits_activations=None,
                                     bits_parameters=None,
                                     bits_accum=32,
                                     mode=LinearQuantMode.ASYMMETRIC_SIGNED,
    ...
    for module_name, input_idxs in input_modules.items():
        denormalized_module_name = utils.denormalize_module_name(model, module_name)
    ...
    for module_name in modules_to_quantize:
        ...
        best_module_override = search_best_local_settings(module, module_name, sg, act_stats, eval_fn,
                                                          ...
                                                          per_channel=args.qe_per_channel)
        best_overrides_dict[normalized_module_name].update(best_module_override)
        utils.yaml_ordered_save('%s.ptq_greedy_search.yaml' % args.arch, best_overrides_dict)
...
if __name__ == "__main__":
    ...
    if args.qe_stats_file:
        ...
        with open(args.qe_stats_file, 'r') as f:
            act_stats = utils.yaml_ordered_load(f)
    else:
        act_stats = None
    ...
    utils.yaml_ordered_save('%s.ptqgs_quantizer_sched_dict.yaml' % args.arch, sched_dict)
