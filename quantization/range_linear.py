...
import modules
import utils.model_transforms as mt
...
class FPWrapper(nn.Module):
    ...
    def forward(self, *input):
        if self.convert_input:
            input = utils.convert_tensors_recursively_to(input, dtype=self.dtype)

        result = self.wrapped_module(*input)
        if self.return_fp32:
            return utils.convert_tensors_recursively_to(result, dtype=torch.float32)
...
class PostTrainLinearQuantizer(Quantizer):
        ...
        if model_activation_stats is not None:
            if isinstance(model_activation_stats, str):
                ...
                with open(model_activation_stats, 'r') as stream:
                    model_activation_stats = utils.utils.yaml_ordered_load(stream)
    ...
    def from_args(cls, model, args):
        ...
        if args.qe_config_file:
            import config 
            return config.config_component_from_file_by_class(model, args.qe_config_file,
    ...
    def save_per_layer_parameters(self, save_dir=''):
        ...
        utils.yaml_ordered_save(save_path, out)
    ...
    def _apply_bn_folding(self, dummy_input):
        ...
        for n, m in named_modules.items():
            ...
            if fused_modules is None:
                continue
            folded_bn_module = utils.normalize_module_name(fused_modules[0])
            ...
            if folded_bn_stats is None:
                continue
            model_stats[utils.normalize_module_name(n)]['output'] = folded_bn_stats['output']
