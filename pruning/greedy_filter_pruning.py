...
from . import L1RankedStructureParameterPruner
import pruning
import utils
import apputils
...
def prune_tensor(param, param_name, fraction_to_prune, zeros_mask_dict):
    ...
    pruner = L1RankedStructureParameterPruner(name=None,
                                                                group_type="Filters",
    ...
    return utils.sparsity_3D(param)


def prune_finetune_test(iteration, model_cpy, pruning_step, test_fn, train_fn,
                        app_args, param_name, effective_train_size):
    pylogger = data_loggers.PythonLogger(msglogger)
    zeros_mask_dict = pruning.create_model_masks_dict(model_cpy)
    ...
    arch = app_args.arch
    pruning.remove_filters(model_cpy, zeros_mask_dict, arch, dataset, optimizer=None)
...
def create_scheduler(model, zeros_mask_dict):
    scheduler = pruning.CompressionScheduler(model)
