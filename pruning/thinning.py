...
def get_normalized_recipe(recipe):
    return ThinningRecipe(
        modules={utils.normalize_module_name(k): v for k, v in recipe.modules.items()},
        parameters={utils.normalize_module_name(k): v for k, v in recipe.parameters.items()},
        )
    ...
    for layer_name, param_name, param in sgraph.named_params_layers():
        if param.dim() in (2, 4):
            ...
            if param.dim() == 2:
                ...
                nonzero_channels = utils.non_zero_channels(param)
                num_nnz_channels = nonzero_channels.nelement()
