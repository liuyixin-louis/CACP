...
# from data_loggers.collector import *
...
class ClassifierCompressor(object):
    ...
    def train_one_epoch(self, epoch, verbose=True):
        ...
        with collectors_context(self.activations_collectors["train"]) as collectors:
            ...
            if verbose:
                utils.log_weights_sparsity(self.model, epoch, [self.tflogger, self.pylogger])
            utils.log_activation_statistics(epoch, "train", loggers=[self.tflogger],
                                                collector=collectors["sparsity"])
            if self.args.masks_sparsity:
                msglogger.info(utils.masks_sparsity_tbl_summary(self.model, 
    ...
    def validate_one_epoch(self, epoch, verbose=True):
        ...
        with collectors_context(self.activations_collectors["valid"]) as collectors:
            ...
            utils.log_activation_statistics(epoch, "valid", loggers=[self.tflogger],
                                                collector=collectors["sparsity"])
        ...
        if verbose:
            ...
            OrderedDict([('Loss', vloss),
                         ('Top1', top1),
                         ('Top5', top5)]))
            utils.log_training_progress(stats, None, epoch, steps_completed=0,
...
def _init_learner(args):
    ...
    if args.compress:
        ...
        compression_scheduler = file_config(model, optimizer, args.compress, compression_scheduler,
...
def create_activation_stats_collectors(model, *phases):
    ...
    genCollectors = lambda: missingdict({
        "sparsity_ofm":      SummaryActivationStatsCollector(model, "sparsity_ofm",
            lambda t: 100 * utils.sparsity(t)),
        "l1_channels":   SummaryActivationStatsCollector(model, "l1_channels",
                                                         utils.activation_channels_l1),
        "apoz_channels": SummaryActivationStatsCollector(model, "apoz_channels",
                                                         utils.activation_channels_apoz),
        "mean_channels": SummaryActivationStatsCollector(model, "mean_channels",
                                                         utils.activation_channels_means),
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    def _log_training_progress():
        ...
        utils.log_training_progress(stats,
                                        params,
...
def test(test_loader, model, criterion, loggers=None, activations_collectors=None, args=None):
    ...
    with collectors_context(activations_collectors["test"]) as collectors:
        ...
        utils.log_activation_statistics(-1, "test", loggers, collector=collectors['sparsity'])
        save_collectors_data(collectors, msglogger.logdir)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1):
    def _log_validation_progress():
        ...
        import utils
        utils.log_training_progress(stats, None, epoch, steps_completed,
...
def quantize_and_test_model(test_loader, model, criterion, args, loggers=None, scheduler=None, save_flag=True):
    ...
    if hasattr(model, 'quantizer_metadata') and \
            model.quantizer_metadata['type'] == quantization.PostTrainLinearQuantizer:
    ...
    if args.device == 'cpu':
        ...
        qe_model = utils.make_non_parallel_copy(model).cpu()
    else:
...
def acts_quant_stats_collection(model, criterion, loggers, args, test_loader=None, save_to_file=False):
    ...
    with utils.get_nonparallel_clone_model(model) as cmodel:
...
def acts_histogram_collection(model, criterion, loggers, args):
    ...
    model = utils.make_non_parallel_copy(model)
