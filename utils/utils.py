...
def sparsity_3D(tensor):
    ...
    l1_norms = utils.norms.filters_lp_norm(tensor, p=1, length_normalized=False)
...
def non_zero_channels(tensor):
    ...
    norms = utils.norms.channels_lp_norm(tensor, p=1)
...
def norm_filters(weights, p=1):
    return utils.norms.filters_lp_norm(weights, p)
...
def _validate_input_shape(dataset, input_shape):
    if dataset:
        try:
            return tuple(classification_get_input_shape(dataset))
        except ValueError:
