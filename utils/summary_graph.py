...
class _ModuleList(object):
    ...
    def _verify_on_insertion(self, module, idx):
        if isinstance(module, nn.ModuleList):
            module = _ModuleList(self._name_for_idx(idx), self.parent_module, module)
        if isinstance(module, _ModuleList):
            if module.parent_module != self.parent_module:
                raise ValueError("When nesting one CACPModuleList within another, both must have the same "
                                 "'parent_module'")
        return module
    ...
    def append(self, module):
        ...
        if not isinstance(module, _ModuleList):
            self.parent_module.add_module(self._name_for_idx(len(self)), module)
        self._modules.append(module)

    def extend(self, modules):
        if not isinstance(modules, Iterable):
            raise TypeError('CACPModuleList.extend must be called with an iterable, but got ' +
...
def _to_modulelist(model):
    """Replaces all instances of torch.nn.ModuleList in a model with ModuleList instances
