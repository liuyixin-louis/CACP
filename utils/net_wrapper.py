...
from utils import normalize_module_name, SummaryGraph
import utils
...
class NetworkWrapper(object):
    ...
    def remove_structures(self, layer_id, fraction_to_prune, prune_what, prune_how,
        ...
        conv_p = utils.utils.model_find_param(self.model, conv_pname)

        msglogger.debug("ADC: trying to remove %.1f%% %s from %s" % (fraction_to_prune*100, prune_what, conv_pname))

        if prune_what == "channels":
            calculate_sparsity = utils.utils.sparsity_ch
            if layer.type == "Linear":
                calculate_sparsity = utils.utils.sparsity_rows
            remove_structures_fn = pruning.remove_channels
            group_type = "Channels"
        elif prune_what == "filters":
            calculate_sparsity = utils.utils.sparsity_3D
            group_type = "Filters"
            remove_structures_fn = pruning.remove_filters
        else:
...
def get_network_details(model, dataset, dependency_type, layers_to_prune=None):
    ...
    dummy_input = utils.utils.get_dummy_input(dataset)
    ...
    for layer_id, (name, m) in enumerate(layers.items()):
        if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Linear):
            ...
            if layers_to_prune is None or name in layers_to_prune:
                ...
                from utils import find_dependencies
